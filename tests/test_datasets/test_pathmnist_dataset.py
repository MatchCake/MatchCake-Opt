...
from matchcake_opt.datasets.pathmnist_dataset import PathMNISTDataset


class TestPathMNISTDataset:
    MOCK_LEN = 10
    ...
    def dataset_instance(self, data_mock, data_dir):
        return PathMNISTDataset(data_dir=data_dir, train=True)

    def test_init(self, data_mock, data_dir):
        dataset = PathMNISTDataset(data_dir=data_dir, train=True)
    ...
    def test_output_shape(self, dataset_instance):
        assert dataset_instance.get_output_shape() == (9,)
